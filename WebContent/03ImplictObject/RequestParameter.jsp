...
<%
	...
	만약 값이 입력되지 않았다마녀 길이가 0인 String 객체를 반환하고
	...
	*/
	...
	if( !(name==null || name.equals("")) ) {
		...
		//전동된 이름이 없다면 아래와 같이 JS를 브라우저에 출력한다.
	...
	getParameter()를 통해 폼 값을 받을 수 있다.
	*/
	String sexStr = request.getParameter("sex");
	/*
	checkbox는 2개 이상의 항목을 선택할 수 있으므로 getPrameterValues()를
	사용한다. 항목 중 체크된 것만 서버로 전송되고, 파라미터명이 틀리거나
	...
	*/
	...
	if(favo != null){
		for(int i=0 ; i<favo.length ; i++){
			...
			문자열의 마지막 부분에 , 를 제거하기 위해 if 문을 통해 
			처리한다.
			*/
			if(i==favo.length-1){
				favStr += favo[i];
...
<%
	...
	/*
	getParameterNames() : 폼 값의 이름을 한꺼번에 받앙ㄹ 때 사용하는
		메소드로 반환타입은 Enumeration 형이다.
	hasMoreElements() : 객체에 남은 폼 값의 이름이 있는지 확인한다.
	nextElement() : 받은 폼 값 중 다음 폼 값의 이름을 반환한다.
	*/
