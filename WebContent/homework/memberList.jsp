...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%
//application 내장객체를 인수로 DAO 객체를 생성한다.(DB연결)
...
int totalPage = (int)Math.ceil((double)totalCount/pageSize); //전체 페이지 갯수 계산
int pageNum =1; //목록 첫 진입시에는 무조건 1페이지로 지정
//만약 파라미터로 전달된 페이지 번호가 있다면
String pageTemp = request.getParameter("pageNum"); 
if(pageTemp != null && !pageTemp.equals("")) //해당 번호로 페이지번호를 지정한다.
...
%>
	<tr align = "center">
		<td><%=vNum %></td>
		<td align="left">
			<a href="View.jsp?num=<%=dto.getNum() %>&<%=queryStr%>"><%=dto.getId() %></a>
		</td>
		<td align="center"><%=dto.getName() %></td>
		<td align="center"><%=dto.getcPhone() %></td>
