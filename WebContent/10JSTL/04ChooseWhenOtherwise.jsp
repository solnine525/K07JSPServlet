...
<%@ taglib prefix="d" uri="myTagLibTld" %>
...
<body>
	...
	<c:if test="${not empty param.kor }">
		...
		<c:set var="avg" value="${(oaram.kor+param.eng+param.math)/3 }" />
			귀하의 평균점수는 ${avg } 입니다.
		<br/>
		<c:choose>
	...
	<c:if test="${not empty param.userid }">
		<c:choose>
			<c:when test="${d:memberLogin(param.userid, param.userpw
							,initParam.JDBCDriver
							,initParam.ConnectionURL) }">${param.userid }님 반갑습니다.
			</c:when>
			<c:otherwise>
				회원이 아닙니다.
			</c:otherwise>
